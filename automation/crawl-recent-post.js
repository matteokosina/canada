...
const logFilePath = path.join(__dirname, '../src/components/RecentPost/recent-post.ts');
...
function getMostRecentPost() {
    try {
        ...
        const filenameWithoutDate = mostRecentFile.replace(/^\d{4}-\d{2}-\d{2}-/, '');
        ...
        return {
            ...
            link: `https://matteokosina.github.io/canada/blog/${filenameWithoutDate.replace('.md', '')}`
        };
    } catch (error) {
...
function writeToLogFile(postData) {
    ...
    const humanDate = dateStr ? new Date(dateStr.replace(/-/g, '/')).toLocaleDateString('de', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
    }) : '';
